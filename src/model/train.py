...
from src.config.config import MODEL_NAME, TRAIN_DATA_PATH, VALIDATION_DATA_PATH, MODEL_SAVE_DIR, MAX_LENGTH, NUM_LABELS
...
def train_model():
    ...
    train_dataset = SentimentDataset(train_df["review"].tolist(), train_df["label"].tolist(), tokenizer, MAX_LENGTH)
    val_dataset = SentimentDataset(val_df["review"].tolist(), val_df["label"].tolist(), tokenizer, MAX_LENGTH)
